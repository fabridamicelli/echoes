...
class GridSearchBase:
    ...
    This class knows how to map the scoring function to gridpoints
    defined by the parameter grid.
    Subclasses implement the actual point evaluation.
    Arbitrary train/test data can be generated by overloading the methods
    _make_data and _eval_point methods as long as you make sure that the
    output from _make_data can be understood by _eval_point. For example,
    you can wrap up an arbitrary task under the methods _make_data and
    ...
    def fit(self, X, y):
        ...
        self.scores_ = Parallel(n_jobs=self.n_jobs, verbose=self.verbose)(
                       delayed(self._eval_gridpoint)(gridpoint, data)
